...
class Page
{
    ...
    public function init()
    {
        ...
        add_action('admin_enqueue_scripts', [$this, 'adminEnqueue'], 30);
    ...
    */
    public function adminBar()
    {
        ...
        $wp_admin_bar->add_menu([
            ...
            'href'  => fdGlobalNonceUrl(admin_url('admin.php?page=' . $this->id)),
        ]);
    }
    ...
    {
        esc_html_e('Congrats! you\'ve created a new page.', 'fastdev');
    }
    ...
    public function displayPage()
    {
        ...
        echo '<h2>' . esc_html($this->getSetting('menu_title')) . '</h2>';
     ...
     * @return void
     */
    final public function adminEnqueue($hook_suffix)
    {
        if ($hook_suffix == 'toplevel_page_' . $this->id) {
            $this->enqueue();
        }
    }
    ...
    protected function showTabs()
    {
        ...
        if (count($this->tabs()) > 1) :
            ...
            foreach ($this->tabs() as $tab_key => $tab) {
                $id           = $tab['id'];
                $label        = $tab['label'];
                $tip          = ! empty($tab['tip']) ? $tab['tip'] : '';
                $active_class = $this->currentTab() == $id ? ' nav-tab-active' : '';

                echo '<a href="' . esc_url(fdGlobalNonceUrl(add_query_arg(
                        ['tab' => $id],
                        $this->getBaseTabUrl()
                    ))) . '" class="fastdev-tab nav-tab ' . sanitize_html_class($active_class) . '">' .
                     esc_html($label) .
                     (! empty($tip) ? ' <span class="dashicons dashicons-editor-help"></span>' : '')
                     . '</a>';

                echo '<div class="fastdev-tab-tip" style="display: none">' . esc_html($tip) . '</div>';
            }
            echo '</h2>';
        endif;
        $this->showSections();
    ...
    protected function getBaseTabUrl()
    {
        ...
        $url  = explode('?', isset($_SERVER['REQUEST_URI']) ? wp_kses_data(wp_unslash($_SERVER['REQUEST_URI'])) : '');
    ...
    protected function showPages()
    {
        foreach ($this->tabs() as $tab_key => $tab) {
            ...
            if ($id == $this->currentTab()) {
                echo '<div class="tab-container ' . sanitize_html_class($active_class) . '">';
    ...
    protected function tabs()
    {
        ...
        $all_tab_sections = apply_filters($this->filter('add_tab_sections'), []);
        $final_tabs       = [];
        $tab_count        = 0;
        ...
        foreach ($all_tabs as $tab_key => $tab) {
            ...
            //Tab tip
            if ( ! empty($tab['tip'])) {
                $final_tabs[$tab_count]['tip'] = $tab['tip'];
            }

            //Custom query args
            $final_tabs[$tab_count]['query_args'] = ! empty($tab['query_args']) ? (array)$tab['query_args'] : [];
    ...
    protected function currentTab()
    {
        ...
        if ( ! wp_verify_nonce(fdGetGlobalNonce(), 'fastdev-admin')) {
            return esc_html($defaultTab);
        }

        return isset($_GET['tab']) && in_array($_GET['tab'], $this->possibleTabs()) ? wp_kses_data(wp_unslash($_GET['tab'])) : esc_html($defaultTab);
    }
            ...
            if ($tab_value['id'] == $this->currentTab()) {
                if (isset($tab_value['sections'])) {
                    if (count($tab_value['sections']) > 0) {
                        ...
                        $remove_query_arg = $this->allQueryArgs(['section']);

                        //Add current tab section
                        $active_class = $this->currentSection() == $tab_value['id'] ? 'current' : '';
                        echo ' <li><a href="' . esc_url(fdGlobalNonceUrl(add_query_arg(['section' => $tab_value['id']],
                                remove_query_arg($remove_query_arg)))) . '" class="' . sanitize_html_class($active_class) . '">
								' . esc_html($default_section_label) . '
							</a> | </li> ';
                        ...
                        foreach ($tab_value['sections'] as $sect_key => $sect_value) {
                            ...
                            echo ' <li><a href="' . esc_url(fdGlobalNonceUrl(add_query_arg(['section' => $sect_value['id']],
                                    remove_query_arg($remove_query_arg)))) . '" class="' . sanitize_html_class($active_class) . '">
								' . esc_html($sect_value['label']) . '
							</a> | </li> ';
                        }
    ...
    protected function currentSection()
    {
        ...
        return isset($_GET['section'])
               && in_array($_GET['section'],
            $this->possibleSections($this->currentTab())) ? wp_kses_data(wp_unslash($_GET['section'])) : $this->currentTab();
    }
        ...
        if ( ! $this->_menuExists($this->id)) { // If menu does not exist, create it.
            ...
            if ($settings['menu_type'] == 'submenu') {
                add_submenu_page(
                    ...
                    $this->id,                     // Menu slug
                    [$this, 'displayPage']    // Calback function to display the page contents
                );
     ...
     */
    public function pageSettings($settings = [])
    {
        // Menu type. `menu` or `submenu`
        if ( ! empty($settings['menu_type']) &&
             in_array($settings['menu_type'], ['menu', 'submenu'])
        ) {
