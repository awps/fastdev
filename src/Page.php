	...
	protected function showTabs() {
		...
		if ( count( $this->tabs() ) > 1 ) :
			...
			foreach ( $this->tabs() as $tab_key => $tab ) {
				...
				$tip_icon     = ! empty( $tip ) ? ' <span class="dashicons dashicons-editor-help"></span>' : '';
				$active_class = $this->currentTab() == $id ? ' nav-tab-active' : '';
				$class        = 'class="fastdev-tab nav-tab' . $active_class . '"';

				echo '<a href="' . add_query_arg(
						array( 'tab' => $id ),
						$this->getBaseTabUrl()
					) . '" ' . $class . '>' . $label . $tip_icon . '</a>';

				echo '<div class="fastdev-tab-tip" style="display: none">' . $tip . '</div>';
			}
			echo '</h2>';
		endif;
		$this->showSections();
